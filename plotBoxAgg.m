<<<<<<< HEAD
function f=plotBoxAgg(B1,B2,B3,B4)%,NN %(A,D) (f,NN,D)
%[a,b]=size(D);
...
%else
    %Abefore=A;
    %f=A(:,1);
    flistAfter=NaN(a*b,lv);%(length(f),lv);
%end
...
%else
    ...
    %[f,g]=finalSizeMulti(gamma,n,nbar,na,NN,NNbar,NNrep,minNind,maxNind,maxN,Kbar,K1,Cbar,betaD,1,2,10^(-16));
    f=flist(:,1);
    flistAfter(1:length(f),1)=f;
    for i=2:lv
        [fnew,NNnew]=aggResult(D,f,v(i)/2);
        flistAfter(1:length(fnew),i)=fnew;
...
%}
figure
maxA=max(max([Aafter;A]));
maxA=min(maxA+.1,1);
minA=min(min([Aafter;A]));
minA=max(minA-.1,0);
subplot(2,1,1)
hold on
fanChart(v,A');
