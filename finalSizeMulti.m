function [f,g]=finalSizeMulti(gamma,n,nbar,na,NN,NNbar,NNrep,Kbar,K1,Cbar,beta,isdual,solvetype,numseed)%,NNbar)
%ZfinalSizeAllMulti2
...
for t=1:lt
    ...
    IC=XODEsolveAllMulti(gamma,NN,n,nbar,NNbar,NNrep,NN0,maxN,cen,D,Z0,beta,ages0,t,t0,tend,zn,phi1,phi2,seed,2); IC=IC./NN0;%New IC here (if ever necessary)
    options = optimset('Display','off');
    funt=@(Zi)solveZi(Zi,Z0,beta,gamma,D,Nages,addbit);
    Zsol=fsolve(funt,IC,options);
    %
    else%solvetype=2/3
    Zsol=XODEsolveAllMulti(gamma,NN,n,nbar,NNbar,NNrep,NN0,maxN,cen,D,Z0,beta,ages0,t,t0,tend,zn,phi1,phi2,seed,solvetype);
    Zsol=Zsol./NN0;
    end
    nu=Zsol-Z0;
...
%%
function f=XODEsolveAllMulti(gamma,NN,n,nbar,NNbar,NNrep,NN0,maxN,cen,D,ic,beta,ages0,tau,t0,tend,zn,phi1,phi2,seed,solvetype)
icR=ic.*NNrep;
y0=[NNbar-icR;zn;icR];
%
if solvetype==2
...
Y=sum(Y,2);
plot(tout,Y,'-','linewidth',lw,'color',[.447,.553,.647]);
