...
NN=1;
demog=1;%Ageing: 1=on
agemix=0;%Total homog: =0; else specify v - twice
tauend=500;
time=(1:tauend);
lt=length(time);
t0=0; tend=5000;
...
numseed=10^(-10); seed=numseed;%*NNprob;
...
for t=1:lt
    if solvetype==1%FSC
    %IC=.5+.1*rand(nbar,1);
    IC=XODEsolveAllMulti(gamma,NN,nbar,NNbar,NNrep,NN0,D,Z0,beta,t0,tend,zn,phi1,phi2,seed,t); IC=IC./NN0;
    if agemix~=1
    ...
    Zsol=XODEsolveAllMulti(gamma,NN,nbar,NNbar,NNrep,NN0,D,Z0,beta,t0,tend,zn,phi1,phi2,seed,t);
    Zsol=Zsol./NN0;
    end
    nu=Zsol-Z0;
...
%%
function f=XODEsolveAllMulti(gamma,NN,nbar,NNbar,NNrep,NN0,D,ic,beta,t0,tend,zn,phi1,phi2,seed,tau)
icR=ic.*NNrep;
y0=[NNbar-icR;zn;icR];
%
%Deterministic:
