...
plotmax=10^4;
logplot=1;
incidence=0;%1 for incidence, 0 for prev.
plotfig=1;
outputGlobal=0;%1 for global incidence, 0 (or ither) for local incidence
%
alpha=1;
a=boxLat;
b=boxLong;
...
%%
function [f,g]=XODEsolveAllMulti(gamma,NN,n,L,beta,Z0,t0,tend,zn,phi1,phi2,seed,alpha,seedvec,plotto,sample,logplotmin,plotmin,plotmax,logplot,incidence,plotfig,outputGlobal)
icR=Z0.*NN;
y0=[NN-icR;zn;icR];
%
...
if incidence==1
    ...
    Y=movmean(Y,3,1);
    Z=yout(:,1:n);
    Z=-diff(Z,1,1).*repmat(diff(tout),1,n);
    Z=movmean(Z,3,1);
    Z=nansum(Z,2);
    Y(1,:)=[]; tout(1)=[];
end
...
if logplot==1
    ...
    if incidence~=1
        semilogy(tvec,Z(1:tend,:),'k--','linewidth',lw);
    end
    hold off
    maxY=max(Ysum);
...
if incidence==1
    ...
    ylabel('Population','FontSize',fs);%Prevalence
end
set(gca,'FontSize',fs);
grid on
grid minor
